...
# secret_des, key, and iv (initialization vector) for aes/des
# =============================================================
...
def encrypt_using_AES(mode, padded_msg, encrypt_std):
...
def decrypt_using_DES3(mode, padded_msg, encrypt_std):
...
def main():
    ...
    if(encrypt_std == 'AES'):
        ...
        padded_msg = validate_input_aes(msg)
        encrypt_using_AES(mode, padded_msg, encrypt_std)
     
    if(encrypt_std == 'DES3'):
        mode = raw_input('What algorithm mode would you want to use (ECB, CBC, CFB, OFB, or CTR)? ')
        mode = mode.upper()
        msg = raw_input('Please input text to encrypt (8 characters): ')
        padded_msg = validate_input_des3(msg)
        encrypt_using_DES3(mode, padded_msg, encrypt_std)
